...
const generateCalendar = (month, year) => {
	...
	// Update the calendar header to display the month and year
    const months = [
	...
	// Get all task due dates for the current month
    const taskDueDates = tasks.map(task => task.dueDate).filter(dueDate => {
    ...
    // Add day cells and highlight days with due tasks
    for (let day = 1; day <= daysInMonth; day++) {
        const dayString = `${year}-${(month + 1).toString().padStart(2, '0')}-${day.toString().padStart(2, '0')}`;

		// Check if the current day has a task with the due date
		const hasTaskDue = taskDueDates.includes(dayString);

		// Add class "highlight" if there's a task due
		calendarHtml += `<td class="calendar-day${hasTaskDue ? " highlight" : ""}" data-date="${dayString}">${day}</td>`;
...
const renderTasksForDate = (date) => {
    const taskListContainer = document.getElementById("task-list-ul");
    taskListContainer.innerHTML = "";  // Clear previous tasks

    const selectedTasks = tasks.filter(task => task.dueDate === date);

    if (selectedTasks.length === 0) {
        taskListContainer.innerHTML = `<li>No tasks for this day</li>`;
    } else {
...
const setupCalendarEventListeners = () => {
    ...
    dayElements.forEach(day => {
        day.addEventListener("click", (e) => {
            ...
            renderTasksForDate(selectedDate); // Display tasks for the clicked date
        });
    });
};
	...
	if (categoryTasks.length === 0) {
      ...
      <div class="delete">
			...
			label.innerHTML = `
              ...
              <p>${task.task}</p>
              <p class="due-date">Due Date: ${task.dueDate ? task.dueDate : 'No due date'}</p>
        `;
...
const addTask = (e) => {
    ...
    const dueDate = document.getElementById("task-date").value;  // Get the selected date

	console.log("Task due date: ", dueDate); // Log the due date
	...
	if (task === "") {
		...
		const newTask = {
			...
			category,
			completed: false,
            dueDate,  // Add due date to the task object
		};
		taskInput.value = "";
		...
		toggleAddTaskForm();
		renderTasks();
		generateCalendar(currentMonth, currentYear); // Re-render calendar to highlight due date
	}
};
